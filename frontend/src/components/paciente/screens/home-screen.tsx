...
import { Flame, Trophy, ChevronRight, Gift, Check } from "lucide-react"
...
/* Anel de progresso animado (SVG) — o foco visual do "centro de progressão". */
      ...
      <svg width={size} height={size} className="-rotate-90">
        ...
        <circle
          ...
          style={{
            ...
            filter: "drop-shadow(0 0 7px rgba(124,255,91,0.55))",
          }}
        />
      </svg>
...
function proximaRecompensa(streak: number, nextLeague: string, pointsToNext: number) {
  if (streak <= 0) return { titulo: "Acenda sua sequência 🔥", detalhe: "hoje" }
  const marco = STREAK_MARCOS.find((m) => m > streak)
  if (marco) {
    const faltam = marco - streak
    return {
      titulo: `Sequência de ${marco} dias`,
      detalhe: faltam === 1 ? "1 dia" : `${faltam} dias`,
    }
  }
  return { titulo: `Chegar em ${nextLeague}`, detalhe: `${pointsToNext.toLocaleString("pt-BR")} pts` }
}

export function HomeScreen({ onNavigate }: { onNavigate: NavigateFn }) {
  const { user, missions, challenges, myPosition } = usePacienteData()
  ...
  const recompensa = proximaRecompensa(user.streak, user.nextLeague, user.pointsToNext)
  ...
  return (
    <div className="space-y-6 px-5 pb-6 pt-7">
      ...
      {/* BLOCO A — Hero de progressão (Progresso + Sequência + Ranking) */}
      <CardNx glow={completo} className="flex flex-col items-center gap-5 py-7">
        <ProgressRing pct={pct}>
          {completo ? (
            <div className="grid size-14 place-items-center rounded-full bg-nx-evo/15 nx-evo-pulse">
              <Check className="size-8 text-nx-evo" strokeWidth={3} />
            </div>
          ) : (
            <>
        ...
        <div className="flex items-center gap-2">
          <ChipNx tone="streak" icon={<Flame className="size-3.5" />}>
            {user.streak} {user.streak === 1 ? "dia" : "dias"}
          </ChipNx>
          {myPosition > 0 && (
            <ChipNx tone="water" icon={<Trophy className="size-3.5" />}>
              #{myPosition} na liga
            </ChipNx>
          )}
          {user.todayPoints > 0 && !completo && (
            <ChipNx tone="evo">+{user.todayPoints} pts hoje</ChipNx>
          )}
        </div>
      </CardNx>

      {/* BLOCO B — Liga & XP (Liga + XP + Próxima liga) */}
      ...
      {/* BLOCO C — Próxima recompensa */}
      <CardNx accent="gold" className="flex items-center gap-4 overflow-hidden">
        <div className="grid size-11 shrink-0 place-items-center rounded-nx-md bg-nx-gold/12">
        ...
        <CardNx className="divide-y divide-nx-border p-0">
          {missions.map((m) => {
            ...
            return (
              ...
              >
                <div
                  className={`grid size-10 shrink-0 place-items-center rounded-nx-md ${
                    m.done ? "bg-nx-evo/15" : "bg-nx-container-high"
                  }`}
                >
                ...
                {m.done ? (
                  <span className="flex items-center gap-1 text-body-sm font-bold text-nx-evo">
                    <Check className="size-4" strokeWidth={3} />+{m.earned}
                  </span>
                ) : (
